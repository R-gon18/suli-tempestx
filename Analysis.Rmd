...
dat %>% filter(CO2_Flux != 0) %>% left_join(collar_map) %>% filter(!is.na(CO2_Flux), !is.na(CH4_Flux), CO2_Flux > 0, CO2_Flux < 25, !is.na(Treatment)) -> by_treatment
...
## CO2 Flux Over Time, Colored By Treatment
```{r}
# Make a quick plot
qplot(Timestamp, CO2_Flux, data = by_treatment, color = Treatment) + ggtitle("CO2 Flux Over Time")
```

This dataset had `r nrow(by_treatment)` observations, from `r min(by_treatment$Timestamp)` to `r max(by_treatment$Timestamp)`

## CH4 Flux Over Time, Colored By Treatment
```{r}
# Make a quick plot
qplot(Timestamp, CH4_Flux, data = by_treatment, color = Treatment) + ggtitle("CH4 Flux Over Time")

last_plot() + ylim(c(-5,5))
```

This dataset had `r nrow(by_treatment)` observations for both CO2 and CH4, from `r min(by_treatment$Timestamp)` to `r max(by_treatment$Timestamp)`
